...
mass = 0.815;
Expi = 1; % Use 18 for ID, then 12, 13, 19, 25 for generalization
...
Fsim3 = Data.Force(1:sum(time <= 0.01),2);
Fsim5 = Data.Force(1:sum(time <= 0.01),3);
...
MoDAL.PlotTSWTFT_Compare(time,Data.Disp(:,2)*1000,time,yCase3(:,1)*1000,0,20,"label", ...
    'Dispmm','title',['Displacement Response, Force = ' num2str(round(max(Fsim3))) ' N'], ...
    'fontSize',14,'legends',{'Experiment','Model'},'timeEnd',45)
set(findall(gcf,'-property','FontSize'),'FontSize',16, 'FontName', 'Times New Roman')

MoDAL.PlotTSWTFT_Compare(time,Data.Disp(:,3)*1000,time,yCase5(:,1)*1000,0,20,"label", ...
